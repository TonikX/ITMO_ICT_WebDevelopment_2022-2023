...
class MyHTTPServer:
    ...
    def handle_get(self):
        ...
        body += '<form><label>Предмет</label><input name="discipline" /><br><br><label>Оценка</label><input name="grade"/><br><br><input type="submit"></form><style>.center{background: #F44336}</style>'
        for subject in self.grades:
            body += f'<div><span>{subject}: {self.grades[subject]}</span></div>'
        body += '</div></body></html>'
    ...
    def handle_post(self, request):
        ...
        if len(discipline) == 0 or grade not in ['1', '2', '3', '4', '5']:
            return self.handle_get()
...
if __name__ == '__main__':
    serv = MyHTTPServer('127.0.0.1', 7780)
    serv.serve_forever()