...
class HotelDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["bookings"] = Booking.objects.filter(
            reservation__in=room_ids, date_end__gte=(datetime.today()-delta))
        context["reservees"] = User.objects.filter(id__in=set(
            [booking["reservee_id"] for booking in context["bookings"].values()]))
        print(f'context["reservees"] = {context["reservees"][0].get_username()}')
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'review_post' in self.request.POST:
            ...
            if review_form.is_valid():
                ...
                review_form.booking = Booking.objects.filter(
                    reservation=room.id, reservee=user).last()
                review_form.save()
